    ...
    const fetchLogement = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setTitre(data.titre || "");
          setType(data.type || "Chambre");
          setPrix(data.prix || "");
          setQuartier(data.quartier || "");
          setPointInteret(data.pointInteret || "");
          setDistanceMetres(data.distanceMetres || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => navigate(`/logements/${id}`), 1000);
      } else {
        setError("Erreur lors de la modification du logement");
      }
    } catch (err) {
      setError("Erreur de connexion au serveur");
    }
